# CLAUDE.md — job-search-workflow (agent instructions)
...
2. **Where the data lives:** run `git status`. If this folder is not a git repository (downloaded as a ZIP), run `git init` and commit the current contents so there is a history; explain in one line that this keeps a record of changes and that their backup is the cloud-drive folder the repo sits in. If it is a repository with no remote, or the remote is the public template, offer once to create a private GitHub repository (`gh repo create <name> --private --source . --remote origin --push`, only if `gh` is installed and logged in; otherwise point to `SETUP.md` step 1) and accept "no" without argument — a local folder is a valid setup.
...
6. **Commit** `profile.md master.md` with message "Profile and master CV"; push only if a remote exists.
